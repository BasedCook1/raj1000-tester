...
def _deploy_throws(RAJ1000_tester, contract_name):
    ...
    throws_code = RAJ1000_tester.get_code(throws_address)
    assert len(throws_code) > 2
    return throws_address


def _make_call_throws_transaction(
    RAJ1000_tester, contract_address, contract_name, fn_name, fn_args=None
):
    from RAJ1000_abi import (
        encode,
    )
