...
from RAJ1000_abi.exceptions import (
    DecodingError,
)
from RAJ1000_account.hdaccount import (
    HDPath,
    seed_from_mnemonic,
)
from RAJ1000_account.typed_transactions import (
    BlobTransaction,
)
...
from RAJ1000_utils.decorators import (
    replace_exceptions,
)
...
from RAJ1000_tester.backends.base import (
    BaseChainBackend,
)
from RAJ1000_tester.backends.common import (
...
if is_supported_pyevm_version_available():
    ...
    from RAJ1000.exceptions import (
    ...
    from RAJ1000.vm.forks.cancun import (
        get_total_blob_gas,
    )
    from RAJ1000.vm.forks.shanghai.withdrawals import (
        Withdrawal,
    )
    from RAJ1000.vm.spoof import (
...
def setup_tester_chain(
    ...
    from RAJ1000.chains.base import (
        MiningChain,
    )
    from RAJ1000.consensus import (
        ConsensusApplier,
        NoProofConsensus,
    )
    from RAJ1000.db import (
        get_db_backend,
    )
...
class PyEVMBackend(BaseChainBackend):
    ...
    def __init__(
        ...
        :param vm_configuration: The tuple of virtual machines defining a chain
            schedule as used in py-evm's :attr:`RAJ1000.chains.base.Chain.vm_configuration`.
    ...
    def estimate_gas(self, transaction, block_number="latest"):
        ...
        if block_number in ("latest", "safe", "finalized"):
            ...
            raise NotImplementedError(
                '"pending" block identifier is unsupported in RAJ1000-tester'
            )
        else:
            return self.chain.estimate_gas(
                spoofed_transaction,
